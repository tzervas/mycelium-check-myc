# Remap Manifest — mycelium-check → check
...
|---|---|---|---|---|---|---|
| `check.bin.myc-check` | Keep | Safe | false | true | `/root/git/isolated/mycelium/crates/mycelium-check/src/bin/myc-check.rs` | structure-preserving 1:1 (DN-109 §5.3-B v0 default: every emitted nodule keeps its source file's mod-tree position, no restructuring proposed) |
| `check` | Keep | Safe | false | true | `/root/git/isolated/mycelium/crates/mycelium-check/src/lib.rs` | structure-preserving 1:1 (DN-109 §5.3-B v0 default: every emitted nodule keeps its source file's mod-tree position, no restructuring proposed) |
